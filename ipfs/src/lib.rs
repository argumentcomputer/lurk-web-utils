...
pub async fn dag_put(host: &String, dag: Ipld) -> Result<String, reqwest::Error> {
  ...
  println!("My Cbor: {:?}", cbor);
  println!("Post request content: {:?}", form);
...
pub async fn dag_get(host: &String, cid: &String) -> Result<Ipld, reqwest::Error> {
  let url = format!(
    "http:/{}{}?arg={}&{}",
    host, "/api/v0/dag/get", cid, "output-codec=dag-cbor"
  );
  let client = reqwest::Client::new();
...
mod test {
  ...
  async fn ipfs_roundtrip() -> Result<(), reqwest::Error> {
    ...
    let cid = dag_put(&String::from("localhost:5001"), ipld.clone()).await?;
